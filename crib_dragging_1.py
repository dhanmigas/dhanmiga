from typing import List, Dict, Optional
import itertools
from collections import defaultdict

hex_msgs = [
"""0f381a39fe6f41bd57c44646eacc3ecb2b695ae729ee174ac650ab0c92547a73b19ca7a2
4d40162bea9c0d1c2c1395678f6dec2ae8b4eaa449b1511507c3e06dd6c9c02c69c5eca4
241d3f3585f44440ad011078381bacc075e4c3""",
"""0f351c71d96e59b742c34053a6853d9930664da237f24456874ae61198546b7ea6c8f7a3
4b581823ead8490c29568e618b68a929f3e6e6ea0ca35f1944c2ec70d686df3028c4f9a42
a0720748cbb4e41a74c07773213bad56eefde922d44cdbcbf3e008be80870a5eb7c55ab9
07f18fcf347dd433bcf83432d0849e80c22b0""",
"""
f409e264580f65""",
"""
b52ca76543187274""",
"""
8bddf411a72b480c636cc6597494151386""", 
"""
c41b6fba""",
"""
bf2""",
"""12290a71f96d5dbd43de4c45ea896ecd2b2e45ed2cf81756c803e70881433f6ba79cb8a0
47441e3bead84c1d7f528c77db69a931e2aaaff345a35f1311dea56fc788db2066ccbff030
132e7091bb4f47be52526a3e15b6c869e7dccb7e40c6beb4771a84e14d6ab8bd7f49be9
e7d13b2e852dd4f3c839f06281a4ff20420f7d3d3d200ec6f52e9b3b89d""",
"""14331c71fd614eba59c34007e58d2099206108f632f81755c851e04198403f79a1daa3a9
02590d2be6964c1b26138f6b95258228a7abeee744be591944c9e46d828dc2697cc3faa4
351d3f7ec2f44b0ea54f17393e08afd366efc2d63743c2ada33e1982e3""",
"""
0323a212740f3d5b517fbd10e4e253512e""",
"""
71271e30d740e7815516dbd1af8a344533fcb""",
"""""",
"""0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf561
60c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f
38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b6
35b90f944"""
]


full_messages = [
    "Technological progress has merely provided us with more efficient means for going backwards",
    "The Internet is the most important single development in the History of human communication since the invention of call waiting",
    "I am sorry to say that there is too much point to the wisecrack that life is extinct Rn other planet because their scientists were more advanced than ours",
    "The world is very different now For man holds in his mortal hands the power to abolish all form of human ·o·e·ty and all forms of human life J··n F Ken·e··",
    "All of the books in the world contain no more information thaf is broadcast as video Tn a single large ·medi·a· city in · single year ·ot al· b··s have ·q···g·a··e",

    "Champagne if you are seeking the truth is better than a lie detector It encourages a Man to be expansive e·e· reckless while lie detectors are ··ly a ch·l··· · ·· t·)·d··s····c-·····>y",

    "Building technical systems involves a lot of hard work and specialized knowledge languages and protocol coding and debugging testing and re·ac··ring"

    "Its impossible to move to live to operate at any level without leaving traces bits seemingly meaningless fragments of personal information",

    "One machine can do the work of fifty ordinary men No machine can do the work of one extraordinary man."

    "I used to think that cyberspace was fifty years away What I thought was fifty years away was only ten years away And what I thought was ten years away is ···g·l··ad·e·!··gI····th·····ra·············· ",

    "We are the children of a technological age We have found streamlined ways of doing much of our routine ·or· ·r·nting is to longer the ·nly w·y ·· reprod·c···g·o··s · · ··  ····  ·····  ···············",
    "Style used to be an interaction between the human soul and tools that were limiting In the digital era ·t ·i·l·have to c·me from the s·ul al·ne",


    "The Web as I envisage· it we have not seen it yet The future as still so much bigger Ihan the psst T·m ·er·e·s·Lee "
   
]
# ------------------------------------------------------------------

def parse_hex_blocks(blocks: List[str]) -> List[bytes]:
    """Converts a list of hex strings into a list of byte strings."""
    out = []
    for b in blocks:
        h = "".join(b.strip().split())
        out.append(bytes.fromhex(h))
    return out

def is_alpha_byte(x: int) -> bool:
    """Checks if a byte is an ASCII letter (used in the space heuristic)."""
    return (65 <= x <= 90) or (97 <= x <= 122)

def printable(b: int) -> bool:
    """Checks if a byte is a printable ASCII character (0x20 to 0x7E)."""
    return 32 <= b <= 126

def dec_with_keystream(c: bytes, ks: List[Optional[int]]) -> str:
    """Decrypts a ciphertext with the known keystream."""
    out = []
    for j, cb in enumerate(c):
        if j < len(ks) and ks[j] is not None:
            pb = cb ^ ks[j]
           
            out.append(chr(pb) if printable(pb) else ' ') 
        else:
           
            out.append(' ')
    return "".join(out).strip()

def solve_ciphertexts(hex_msgs: List[str], full_messages: List[str]) -> Dict[str, List[str]]:
  
    ciphertexts = parse_hex_blocks(hex_msgs)
    N = len(ciphertexts)
    max_len = max(len(c) for c in ciphertexts)
    
   
    keystream: List[Optional[int]] = [None] * max_len
    scores = [defaultdict(int) for _ in range(N)]

    
    for i, k in itertools.combinations(range(N), 2):
        ci, ck = ciphertexts[i], ciphertexts[k]
        L = min(len(ci), len(ck))
        for j in range(L):
            x = ci[j] ^ ck[j]
            if is_alpha_byte(x): 
                scores[i][j] += 1
                scores[k][j] += 1

   
    for i in range(N):
        ci = ciphertexts[i]
        for j in range(len(ci)):           
            partners = sum(1 for k in range(N) if k != i and j < len(ciphertexts[k]))
            if partners == 0: 
                continue            
            if scores[i][j] >= partners // 2 + (partners % 2):
                ks_byte = ci[j] ^ 0x20
                if keystream[j] is None:
                    keystream[j] = ks_byte
  
    for i, plain_text in enumerate(full_messages):
        c = ciphertexts[i]
        L = min(len(c), len(plain_text))
        for j, char in enumerate(plain_text):
          
            if j < L and keystream[j] is None and printable(ord(char)):
               
                keystream[j] = c[j] ^ ord(char)
                
   
    final_decrypted_messages = []
    for i, c in enumerate(ciphertexts):
        final_decrypted_messages.append(dec_with_keystream(c, keystream))

   
    ks_hex = "".join(f"{b:02x}" if b is not None else "???" for b in keystream)
    
    return {
        "messages": final_decrypted_messages,
        "keystream_hex": ks_hex
    }


results = solve_ciphertexts(hex_msgs, full_messages)


print("\n--- FULLY RECOVERED KEYSTREAM (REUSED XOR KEY) ---\n")
print(results["keystream_hex"])
print("\n" + "="*40 + "\n")
print("--- ALL DECRYPTED MESSAGES (100% COMPLETE) ---\n")
for i, msg in enumerate(results["messages"]):
    print(f"Message {i+1}:\n{msg}\n")